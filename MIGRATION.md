...
```
TgXUIMgr/
├── main.py               # Бот: команды, диалоги, задачи
├── config.py             # Работа с config.yml, валидация URL
├── database.py           # SQLite: трафик, привязки, пользователи бота
├── xui_api.py            # HTTP-клиент панели 3x-ui 3.8.x
├── query_logic.py        # Поиск клиента по email
│
...
| Файл | Что делает |
|------|-----------|
| `main.py` | Регистрирует команды, диалоги `/setting` и `/addclient`, ежедневные задачи |
| `config.py` | Читает и пишет `config.yml`, валидирует URL панелей |
| `database.py` | Три таблицы: `traffic_records`, `client_bindings`, `bot_users`, `notification_log` |
| `xui_api.py` | Общается с панелью 3x-ui по HTTPS |
| `query_logic.py` | Общая функция «найти клиента по email» |
