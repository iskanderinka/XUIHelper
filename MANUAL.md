...
продление. Ведёт статистику трафика и присылает ежедневные отчёты.
...
| `helpers.py` | Общие утилиты: форматирование, валидация, клавиатуры, работа с панелями, датами |
...
# Часовой пояс сервиса (планировщик, даты, «конец дня»)
...
8. **Бот** создаёт клиента в панели, сохраняет связку, отправляет клиенту
...
Бот автоматически **продлит срок** на число дней паузы. Для бессрочных подписок
срок не меняется.
...
| `expiry_notification_job` | Ежедневно в 9:00 | Напоминания клиентам (7 и 3 дня), алерт админам после истечения |

Время задач считается в часовом поясе из `config.yml` (по умолчанию `Asia/Hong_Kong`).
...
**В: Что если автоматика сброса трафика не сработает?**
О: Автосброс отключён. Если нужно обнулить клиенту счётчик — сделай это руками в панели.
